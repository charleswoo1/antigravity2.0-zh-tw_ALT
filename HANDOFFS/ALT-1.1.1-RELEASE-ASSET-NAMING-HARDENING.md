...
9. 新增 `tests/release-assets.test.js`，並納入 `npm run check:packaging`。
...
- PR 初始 head 的 `Core / Ubuntu` 已通過 smoke/process-safety、packaging/policy tests 與 production dependency audit；最終 head 由 PR CI 再次完整驗證。
