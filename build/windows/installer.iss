...
begin
  ...
  if ResultCode <> 0 then begin
    ...
    SummaryText := '';
    LoadStringFromFile(SummaryPath, SummaryText);
    if SummaryText = '' then
      SummaryText := '原因：ALT 相容性 preflight 失敗。' + #13#10 + 'Antigravity 未被修改。';
    Result := String(SummaryText) + #13#10 + '詳細記錄：' + LogPath;
  end;
#endif
end;
...
begin
  ...
  if ResultCode <> 0 then begin
    ...
    LoadStringFromFile(SummaryPath, SummaryText);
    if SummaryText = '' then
      SummaryText := '原因：中文化引擎執行失敗，請查看完整記錄。' + #13#10 + 'Antigravity 未被修改。';
    FailSetup('ALT {#Mode} 失敗。' + #13#10 + #13#10 + String(SummaryText) + #13#10 + '詳細記錄：' + LogPath, ResultCode);
    exit;
  end;
